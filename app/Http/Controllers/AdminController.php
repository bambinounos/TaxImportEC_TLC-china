...
class AdminController extends Controller
{
    ...
    public function storeTariffCode(Request $request)
    {
        $request->validate([
            ...
            'hierarchy_level' => 'nullable|integer|in:4,6,10',
    ...
    public function updateTariffCode(Request $request, string $hsCode)
    {
        ...
        $tariffCode->update([
            ...
            'unit' => $request->unit ?? $tariffCode->unit,
            'hierarchy_level' => $request->hierarchy_level ?? $tariffCode->hierarchy_level,
            'parent_code' => $request->parent_code ?? $tariffCode->parent_code,
            'order_number' => $request->order_number ?? $tariffCode->order_number,
            'has_ice' => $request->boolean('has_ice', false),
            'is_active' => $request->boolean('is_active', true)
        ]);
